...
instellingen_image_path = r'C:\Users\joshu\Downloads\RLSystems-main\RLSystems-main\SettingsV1.1.png'  # Ensure the path is correctly escaped
start_image_path = r'C:\Users\joshu\Downloads\RLSystems-main\RLSystems-main\StartIcon.png'  # Ensure the path is correctly escaped
    ...
    with ui.column().classes('left-splitter-panel'):
        with ui.tabs().props('vertical').classes('w-full') as tabs:
            with ui.tab('Starts').classes('tab-with-image'):
                ...
                ui.label('Starts').classes('tab-label')
            with ui.tab('Instellingen').classes('tab-with-image'):
                ui.image(instellingen_image_path).classes('icon')
                ui.label('Instellingen').classes('tab-label')
            with ui.tab('Movies').classes('tab-with-image'):
                ui.icon('movie').classes('icon')
                ui.label('Movies').classes('tab-label')
    with splitter.after:
        with ui.tab_panels(tabs).props('vertical').classes('w-full h-full'):
            ...
            with ui.tab_panel('Movies').classes('splitter-content'):
